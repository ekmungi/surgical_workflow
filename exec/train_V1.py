...
def train(optimizer_options, data_options, logger_options, model_options, scheduler_options):#, results_path=None):
    ...
    for iFold, (train_loader, val_loader) in enumerate(folds_pbar): #= next(kfoldWorkflowSet)
        ...
        cycle_length = scheduler_options['cycle_length'] if scheduler_options['cycle_length']>0 else len(train_loader)
        scheduler(optimizer, cycle_length)

        trainer = Engine(model, optimizer, criterion_CE, scheduler, train_loader, 
        ...
        for epoch in epoch_pbar:
            ...
            iteration_pbar = ProgressBar(train_loader, desc="Iteration", pb_len=optimizer_options['max_iterations'])
            ...
            for iteration, (images, phase_annotations) in enumerate(iteration_pbar):
                ...
                if (optimizer_options["validation_interval"] > 0):
                    if ((iteration+1) % optimizer_options["validation_interval"] == 0) and (iteration>0):
                ...
                if (iteration % logger_options["vislogger_interval"] == 0):
                    vis.line(X=np.array([epoch + (iteration/iteration_pbar.total)]), 
                                Y=np.array([epoch_training_avg_loss.get_value()[0]]),
