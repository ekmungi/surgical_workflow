...
# class WorkflowBatchGenerator(object):
...
#         # phase_annotations = np.vstack(phase_annotations)
        
        
#         # return {'data': images, 'target': phase_annotations}
...
class kFoldWorkflowSplitMT(DatasetFolder):
    ...
    def __init__(self, base_path, image_transform=None, n_folds=4,
                    ...
                    batch_size=4, n_batches=100, num_workers=1, 
    ...
    def __next__(self):
        ...
        if self.n <= self.n_folds:
            ...
            train_batchloader = ia.BatchLoader(load_batches(train_datasets, self.batch_size, self.n_batches))#, nb_workers=self.num_workers)
            valid_batchloader = ia.BatchLoader(load_batches(valid_datasets, self.batch_size, self.n_batches))#, nb_workers=self.num_workers)

            train_bg_augmenter = ia.BackgroundAugmenter(train_batchloader, self.image_transform, nb_workers=self.num_workers)
            valid_bg_augmenter = ia.BackgroundAugmenter(valid_batchloader, self.image_transform, nb_workers=self.num_workers)
