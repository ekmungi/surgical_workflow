    ...
    def get_next_loader(self, train=True, build_new=False):
        ...
        if self.n <= self.n_folds:
            ...
            if self.bg_augmenter is None:
                if train:
                    self.bg_augmenter = MultiThreadedAugmenter(self.train_batchgenerator, self.image_transform, 
                                                                self.num_workers, pin_memory=True)
                else:
                    self.bg_augmenter = MultiThreadedAugmenter(self.valid_batchgenerator, self.image_transform, 
                                                                self.num_workers, pin_memory=True)
            else:
                if train:
